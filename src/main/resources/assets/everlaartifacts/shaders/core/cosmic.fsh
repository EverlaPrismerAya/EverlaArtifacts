...
void main (void)
{
    ...
    // background color - deep dark tone
    vec4 col = vec4(0.05, 0.0, 0.08, 1.0);

    float pulse = mod(time, 400) / 400.0;

    col.g = sin(pulse * M_PI * 2) * 0.05 + 0.1;
    col.b = cos(pulse * M_PI * 2) * 0.05 + 0.15;
