package net.everla.everlaartifacts.server.handlers.items.penis_music;
...
public class PenisMusicExplosionHandler {
    @SubscribeEvent
    public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        if (event.getHand() != event.getEntity().getUsedItemHand())
            return;
        handlePenisMusicExplosion(event, event.getLevel(), event.getPos().getX(), event.getPos().getY(), event.getPos().getZ(), event.getEntity());
    }

    public static void handlePenisMusicExplosion(LevelAccessor world, double x, double y, double z, Entity entity) {
        handlePenisMusicExplosion(null, world, x, y, z, entity);
    }

    private static void handlePenisMusicExplosion(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
