package net.everla.everlaartifacts.common.handlers.items.glasses;
...
public class GlassesCuriosHandler {
    ...
    public static void onAttachCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
        ...
        if (stack.getItem() != EverlaartifactsModItems.GLASSES.get()) {
            return;
        }
    ...
    private static class CurioProvider implements ICapabilityProvider {
        ...
        public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
            if (cap == CuriosCapability.ITEM) {
                return LazyOptional.of(() -> (ICurio) new GlassesCurio(stack)).cast();
            }
    ...
    private static class GlassesCurio implements ICurio {
        private final ItemStack stack;

        GlassesCurio(ItemStack stack) {
            this.stack = stack;
        }
