...
public class PerformanceBasedThingsHandler {
    ...
    private static double doGetPlayerPerformanceScore(Player player) {
        ...
        if (player != null && player.level() != null && !player.level().isClientSide()) {
            // 在服务端检查游戏规则
            boolean forceUseTruePerformance = player.level().getGameRules().getBoolean(ForceUseTruePerformance.FORCE_USE_TRUE_PERFORMANCE);
            if (forceUseTruePerformance) {
                // 如果启用了强制使用真实性能，返回当前机器的真实性能评分
                double truePerformanceScore = PerformanceMetrics.calculateTotalScore(
                ...
                try {
                    ...
                    if (mcPlayer != null) {
                        ...
                        if (actualPlayerUUID.equals(player.getUUID())) {
                            ...
                            double clientScore = PerformanceMetrics.getClientPerformanceScore();
                            return clientScore;
                        }
                    }
    ...
    public static void onItemTooltip(ItemTooltipEvent event) {
        ...
        if (moneyBurnersCreedLevel > 0 || scrapyardScroungerLevel > 0) {
            ...
            if (net.minecraftforge.fml.loading.FMLEnvironment.dist.isClient()) {
                performanceScore = PerformanceMetrics.getClientPerformanceScore();
            }
