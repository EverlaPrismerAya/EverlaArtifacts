package net.everla.everlaartifacts.common.handlers.items.atm_ring;
...
public class AtmRingCuriosHandler {
    ...
    public static void onAttachCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
        ...
        if (stack.getItem() != EverlaartifactsModItems.ATM_RING.get()) {
            return;
        }
    ...
    private static class CurioProvider implements ICapabilityProvider {
        ...
        public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
            if (cap == CuriosCapability.ITEM) {
                return LazyOptional.of(() -> (ICurio) new AtmCurio(stack)).cast();
            }
    ...
    private static class AtmCurio implements ICurio {
        private final ItemStack stack;

        AtmCurio(ItemStack stack) {
            this.stack = stack;
        }
