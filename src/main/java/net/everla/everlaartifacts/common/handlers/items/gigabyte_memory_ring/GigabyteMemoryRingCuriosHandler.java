package net.everla.everlaartifacts.common.handlers.items.gigabyte_memory_ring;
...
public class GigabyteMemoryRingCuriosHandler {
    ...
    public static void onAttachCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
        ...
        if (stack.getItem() != EverlaartifactsModItems.GIGABYTE_MEMORY_RING.get()) {
            return;
        }
    ...
    private static class CurioProvider implements ICapabilityProvider {
        ...
        public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
            if (cap == CuriosCapability.ITEM) {
                return LazyOptional.of(() -> (ICurio) new RingCurio(stack)).cast();
            }
    ...
    private static class RingCurio implements ICurio {
        private final ItemStack stack;

        RingCurio(ItemStack stack) {
            this.stack = stack;
        }
