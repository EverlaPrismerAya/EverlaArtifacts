...
public class HomaActiveParticleHandler {
    ...
    public static void onLivingUpdate(LivingEvent.LivingTickEvent event) {
        ...
        // 检查实体是否具有HomaActive状态效果
        if (entity.hasEffect(net.everla.everlaartifacts.init.EverlaartifactsModMobEffects.HOMA_ACTIVE.get())) {
            // 每隔几tick生成一次粒子，以减少频率
            if (entity.level().isClientSide() || entity.level().getGameTime() % 4 == 0) {
                // 在实体边界框内随机位置生成粒子
                // 50% GoldButterfly, 50% FireButterfly
